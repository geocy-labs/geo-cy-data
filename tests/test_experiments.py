...
from geocydata.experiments.data import load_bundle_dataset, prepare_experiment_matrix
...
def test_experiment_bundle_loading(tmp_path: Path) -> None:
    ...
    matrix = prepare_experiment_matrix(dataset, model_name="local")
...
def test_local_model_run_smoke(tmp_path: Path) -> None:
    ...
    metrics = run_experiment(bundle_dir=bundle_dir, model_name="local", out_dir=run_dir, seed=7)
...
def test_global_model_run_smoke(tmp_path: Path) -> None:
    ...
    metrics = run_experiment(bundle_dir=bundle_dir, model_name="global", out_dir=run_dir, seed=7)
...
def test_compare_writes_comparison_file(tmp_path: Path) -> None:
    ...
    comparison = compare_experiments(bundle_dir=bundle_dir, out_dir=comparison_dir, seed=7)
